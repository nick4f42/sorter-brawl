...
namespace SorterBrawl.Frames.Stylers
{
    class BarStyler : IStyler
    {
        ...
        {
            int x = (int)((double)data.i / data.length * data.width);

            int rectWidth = (int)((double)(data.i + 1) / data.length * data.width) - x;

            int rectHeight = data.height * (data.value - data.minValue + 1) / (data.maxValue - data.minValue + 1);
            ...
            lock (data.graphics)
                data.graphics.FillRectangle(new SolidBrush(color),
                  new Rectangle(x, data.height - rectHeight, rectWidth, rectHeight));
        }
    }
}
